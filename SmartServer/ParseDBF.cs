...
public class ParseDBF
{
    ...
    public static DataTable ReadDBF(string dbfFile)
    {
        ...
        try
        {
            ...
            {
				number = Encoding.GetEncoding("windows-874").GetString(recReader.ReadBytes(field.fieldLen));
            ...
            for (int counter = 0; counter <= header.numRecords - 1; counter++)
            {
                ...
                foreach (FieldDescriptor field in fields)
                {
                    switch (field.fieldType)
                    {
						...
						case 'C': // String
                            row[fieldIndex] = Encoding.GetEncoding("windows-874").GetString(recReader.ReadBytes(field.fieldLen));
							break;

						case 'D': // Date (YYYYMMDD)
                            year = Encoding.GetEncoding("windows-874").GetString(recReader.ReadBytes(4));
                            month = Encoding.GetEncoding("windows-874").GetString(recReader.ReadBytes(2));
                            day = Encoding.GetEncoding("windows-874").GetString(recReader.ReadBytes(2));
