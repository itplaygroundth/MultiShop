...
namespace MultiShop
{
    public partial class SearchForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        void work_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                dt = LoadData(string.Format("SELECT DISTINCT BCITEM.CODE,NAME1,STOCKQTY,BCITEMUNIT.NAME AS UNITNAME,SALEPRICE{1}*((100+{2})/100) AS SALEPRICE FROM BCITEM,BCITEMUNIT  WHERE  ((BCITEM.CODE  LIKE '%{0}%' ) OR (NAME1  LIKE '%{0}%' ) OR (BCITEMUNIT.NAME  LIKE '%{0}%' )) AND BCITEM.DEFSTKUNITCODE=BCITEMUNIT.CODE AND  BCITEM.ACTIVESTATUS <>2 AND BCITEM.ACTIVESTATUS <>3  AND  (BCITEM.Name1 Like '%{0}%' Or BCITEM.Code Like '%{0}%') ORDER BY BCITEM.CODE", textSearch.Text.ToString(),MainForm._appconf.saleprice,MainForm._appconf.gp));
            }
            catch (Exception ex)
            {
        ...
        private void preData()
        {
            ...
            dt = LoadData(string.Format("SELECT DISTINCT BCITEM.CODE,NAME1,STOCKQTY,BCITEMUNIT.NAME AS UNITNAME,SALEPRICE{0}*((100+{1})/100) AS SALEPRICE FROM BCITEM,BCITEMUNIT  WHERE ((BCITEM.CODE  LIKE '%%' ) OR (NAME1  LIKE '%%' ) OR (BCITEMUNIT.NAME  LIKE '%%' )) AND BCITEM.DEFSTKUNITCODE=BCITEMUNIT.CODE AND  BCITEM.ACTIVESTATUS <>2 AND BCITEM.ACTIVESTATUS <>3  ORDER BY BCITEM.CODE",MainForm._appconf.saleprice,MainForm._appconf.gp));
            iAllRows = Convert.ToInt16(Math.Ceiling(Convert.ToDecimal(dt.Rows.Count / iPageSize))) + 1;
            //gridControl2.DataSource = goToPage(1);
        }
